...
final class ConfigurationParser
{
	...
	void parse(InputStream in, KarmaClassLoaderRepository repository) throws IOException {
	...
	private void parseEntry(String targetClassLoaderName, String entry) throws IOException, FileNotFoundException {
	...
	/**
	 * A directory can be added to the classloader with an entry like
	 * "dir = <directory-name>".
	 */
	private void parseDirEntry(String targetClassLoaderName, String entry) throws FileNotFoundException {
		int i = -1;
...
//			System.out.println("Added directory \"" + dirName + "\" to KarmaClassLoader \"" + targetClassLoaderName + "\"");
	...
	/**
	 * A file can be added to the classloader with an entry like
	 * "file = <file-name>".
	 */
	private void parseFileEntry(String targetClassLoaderName, String entry) throws FileNotFoundException {
		int i = -1;
...
//			System.out.println("Added file \"" + fileName + "\" to KarmaClassLoader \"" + targetClassLoaderName + "\"");
	...
	/**
	 * A jar can be added to the classloader with an entry like
	 * "id=<id>, version=<version>" or "id=<id>, jar=<jar>"
	 */
	private void parseIdEntry(String targetClassLoaderName, String entry) throws FileNotFoundException, IOException {
		...
		if (((i = entry.indexOf(",")) != -1) && !entry.endsWith(",")) {
			...
			if (((i = versionOrJarPart.indexOf("=")) != -1) && !versionOrJarPart.endsWith("=")) {
				versionOrJar = versionOrJarPart.substring(i + 1).trim();
			}


			if ((versionOrJarPart.startsWith("version") || versionOrJarPart.startsWith("jar")) &&
