...
final class RemoteRepository
{
	...
	/**
	 * Get the remote repository. The locations can be specified in the following ways
	 * 1) From a file loaded from the classpath called "karma.repository.remote"
	 * 2) In a file $HOME/.karma/karma.repository.remote (overrides the file loaded from the classpath)
	 * 3) The system property karma.repository.remote (overrides all)
	 * remote locations will be searched in reverse order, so the repository defined as a system
	 * property will be searched first, next the ones specified in a local configuration file
	 * and last the ones specified in the file available in the classpath.
	 */
	...
	private RemoteRepository() throws IOException {
		...
		if (systemProperty != null) {
			if (!remoteRepositories.contains(systemProperty)) {
				...
				System.out.println("Adding a remote repository \"" + systemProperty + "\"");
			}
		}

		// First try to load a list of repositories from the classpath
		//
		if ((in = this.getClass().getClassLoader().getResourceAsStream("karma.repository.remote")) != null) {
			readRepositories(in);
			in.close();
		}
		...
		//
		File f = new File
			(System.getProperty("user.home") +  File.separator + ".karma"+ File.separator + "karma.repository.remote");
		if (f.exists()) {
	...
	File getJarByName(String id, String name)
		throws IOException
	{
		...
		try {
			...
			while ((i < remoteRepositories.size()) && !retrievedJar) {
				try {
					...
					// Try to get a MD5 file from the location from which the jar was obtained
					// If the MD5 exists and we were able to retrieve it, we need to compare
					// the MD5 of the jar with the retrieved MD5. If they are equal, than
					// the retrieval of the jar was successfull, otherwise it failed.
					//
					String referenceMd5 = null;
					if ((referenceMd5 = getMd5ForJar(urlString + ".md5")) != null) {
						if (!referenceMd5.equals(md5))
						{
							throw new IOException("MD5 check failed, found \"" + md5 + "\" should have been \"" +
								referenceMd5 + "\"");
						} else {
					...
					System.out.println("Failed to retrieve jar from url \"" + urlString + "\" due to " +
						"\"" + ioe.getMessage() + "\"");
				}
				i += 1;
			}
	...
	private static String getMd5ForJar(String md5UrlString) {
		...
		try {
			...
			referenceMd5 = (new BufferedReader(new FileReader(tmpFile))).readLine().trim();
