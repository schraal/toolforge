...
final class HttpUtil
{
	 ...
	 * @return A string representation of the MD5 MessageDigest of the url retrieved, or
	 */
	static String get(URL url, File dest) throws IOException {
		...
		try {
			...
			if (connection.getResponseCode() != HttpURLConnection.HTTP_OK) {
				throw new IOException("Failed to retrieve remote file (HTTP response code = " +
			...
			while ((i = in.read(buffer, 0, BUFFER_SIZE)) != -1) {
				...
				if (n >= BUFFER_SIZE) {
					if (!headerIsPrinted) {
						System.out.print("Progress (in blocks of " + BUFFER_SIZE + " bytes):");
