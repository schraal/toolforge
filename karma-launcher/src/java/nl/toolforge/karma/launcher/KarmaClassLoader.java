...
/**
 * The KarmaClassLoader is a kind of URL class loader, with the extra ability to
 * add directories or (jar) files after creation of the class loader with the
 * addRepository methods.
 * 
 * @author W.M. Oosterom
 */
public final class KarmaClassLoader extends URLClassLoader {
     ...
     * file and ends with ".jar" it is added to the classpath. If the file is a
     * directory the directory is added to the classpath. If the directory
     * contains jars files, also the jar files are added to the classpath
     * 
     * @param repository
    ...
    public void addRepository(File repository) throws FileNotFoundException {
        ...
        if (!repository.exists()) {
            throw new FileNotFoundException(repository.getName()
                    + " does not exist.");
        }
        ...
        try {
            ...
            // The directory might contain jar files, these should also be
            // added,
            // so try to find jars
            //
