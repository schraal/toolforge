...
public class LoggedDataInputStream extends FilterInputStream {
  ...
  public String readLine() throws IOException {
    ...
    loop: while (true) {
      ...
      // This thing can hang, which is not something we want. We ensure that one response line
      // from CVS doesn't take longer than 1000 ms. After that, we throw an IOException.
      //
      if ((System.currentTimeMillis() - start) > MAX_DURATION) {
        throw new IOException("Time out when waiting for a CVS response.");
      }
